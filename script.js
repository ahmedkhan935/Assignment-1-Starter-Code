...
// jobList.on("click", function(event) {
...
// // Function to add a tag to the search bar
...
//     if(flag)
//         return;    
//     // Create a tag element with a cross sign
...
//     const children=$(selectedTagsContainer).children().toArray();
   
//         var filterTags=[];
//         children.forEach(child=>{
//             if(filterTags.includes(child.id)==false){
//                 filterTags.push(child.id);
//             }
...
//     }
// }
// async function generateFilteredJobListings(filterTags) {
//     if(jobData.length === 0)
//         await getJobs();
//     $("#job-list").html("");
//     jobData.forEach(job => {
...
//             if(job.role==tag || job.level==tag || job.languages.includes(tag) || job.tools.includes(tag))
//                 flag=true;
//         });
//         if(flag){
//                 const listItem = $("<li>").addClass("job-item");
